...
def main():
    ...
    for item in results:
        ...
        auc_str, yest_str = f"{int(item['auc'])}万", f"{item['last_amt']/10000:.1f}亿" if item['last_amt']>10000 else f"{int(item['last_amt'])}万"
        ...
        prefix = f"{Back.RED}{Fore.WHITE} 💼持仓 {Style.RESET_ALL} " if item['is_holding'] else (f"{Back.YELLOW}{Fore.BLACK} ⭐关注 {Style.RESET_ALL} " if item['is_focus'] else "")
        print(f"{item['code']:<8} {item['name']:<6} {c_open}{item['open_pct']:>6.2f}{Style.RESET_ALL}  {c_real}{item['real_pct']:>6.2f}{Style.RESET_ALL}  {c_yest}{item['yest_pct']:>6.2f}{Style.RESET_ALL}   {Fore.YELLOW}{auc_str:<8}{Style.RESET_ALL} {boards_str:<5} {mv_str:<8} {yest_str:<8} {item['sector_info']:<12} {prefix}{item['decision']}")
