...
class StockTagger:
    ...
    def get_tags(self, item, strategies_hit_tags):
        ...
        yest_pct = item.get('yest_pct', item.get('today_pct', 0.0))  # 昨天涨幅

        # 1. 绝对金额过亿，主力入场标志
        if auc_amt > 100000000:
            hit_tags.append("💰竞价过亿")

        # 2. 竞价达标与超预期 (设置绝对门槛：至少1000万以上)
        if auc_amt >= 10000000:
            if auc_ratio >= 0.10:
                if auc_pct > 0:
                    # 高开(红盘)，结合昨日弱势/分歧为超预期
                    if yest_pct < 5.0 and auc_pct > 1.5:
                        ...
                        hit_tags.append("🔥竞价爆量抢筹")
                elif auc_pct <= 0.0:
                    # ✅ 优化：冰点反核战法识别 (区分恶意砸盘与恐慌承接)
                    # 昨日大跌(<-5%)，今日水下开但跌幅可控(>-4%)，且爆量
                    if yest_pct <= -5.0 and auc_pct >= -4.0:
                        hit_tags.append("🌟深水爆量承接")
                        force_select = True  # 核心反核信号，强制送入监控池
                    elif auc_pct < -2.0:
                        # 只有真的深水爆量且没有昨日暴跌基础的，才算砸盘
                        hit_tags.append("💣竞价爆量砸盘")

            elif auc_ratio >= 0.05:
                if auc_pct > 0:
                    hit_tags.append("⚡竞价达标")
                elif auc_pct <= 0.0 and yest_pct <= -5.0 and auc_pct >= -4.0:
                    # 适当放宽量比(>=0.05)但金额达标，同样捕捉深水承接
                    hit_tags.append("🌟深水爆量承接")
                    force_select = True
        ...
        close_price = item.get('price', 0.0)
